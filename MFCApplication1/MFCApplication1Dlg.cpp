...
void CMFCApplication1Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON1, m_lst);
	//  DDX_Control(pDX, IDC_BUTTON2, m_stplst);
...
void CMFCApplication1Dlg::OnBnClickedButton1()
{
	...
	u_short port;
	port = _ttoi(m_port);
	//m_stplst.EnableWindow(true);
	m_lst.EnableWindow(false);

	// 获取选择的是TCP or UDP
	int cb_index = m_cb1.GetCurSel();  // 这个函数用于得到用户选择的是下拉列表中的第几行，第一行的话，返回0
	if (cb_index == 0)
		zlog_info(c, "尝试启动监听，端口号：%d，协议：TCP", port);
	else
		zlog_info(c, "尝试启动监听，端口号：%d，协议：UDP", port);
	m_ser.m_server(port, cb_index);
	zlog_info(c, "测试完毕");

	//m_stplst.EnableWindow(false);//重新激活监听按钮
