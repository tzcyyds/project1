...
BOOL CMFCApplication2App::InitInstance()
{
	...
	if (!c) {
		printf("get zlog_cat fail\n");
		zlog_fini();
		return -2;
	}
	data = zlog_get_category("my_data");
	if (!data) {
		printf("get zlog_data fail\n");
		zlog_fini();
		return -2;
	}
	zlog_info(c, "ZLOG_Init:Sucessful");
	...
	if (!AfxSocketInit())
	{
		...
		zlog_error(c, "MFC_Init:IDP_SOCKETS_INIT_FAILED");
		zlog_fini();
		return FALSE;
	}
	...
	{
		zlog_warn(c, "MFC_Init:对话框创建失败，应用程序将意外终止。");
